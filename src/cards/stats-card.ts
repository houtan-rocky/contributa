...
const createTextNode = ({ imageBase64, name, rank, index, height, url }) => {
  ...
  const itemUrl = url || '#';
  return `
  <a xlink:href="${itemUrl}" xlink:title="Link to ${itemUrl}" target="_blank">
...
export const renderContributorStatsCard = async (
  ...
  let statItems = Object.keys(transformedContributorStats).map((key, index) =>
    ...
    createTextNode({
      ...
      index,
      lheight,
      url: transformedContributorStats[key].url,
    }),
  );
